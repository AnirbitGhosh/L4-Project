...
model = pretrained_network.PretrainedNet().to(device)
...
def main():
    ...
    # Save
    # np.save('D:/PCAM DATA/WSI/pretrained_predictions/figures/{}.npy'.format(args.file.split('.')[0]), tumor_map)
    np.save('D:/PCAM DATA/WSI/pretrained_predictions/figures/02.npy', tumor_map)
    
    cmapper = cm.get_cmap('plasma')
    colorized = Image.fromarray(np.uint8(cmapper(np.clip(tumor_map, 0, 1)) * 255))
    colorized.save('D:/PCAM DATA/WSI/pretrained_predictions/02.tif')
    # colorized.save('D:/PCAM DATA/WSI/pretrained_predictions/{}.tif'.format(args.file.split('.')[0]))

...
\begin{lstlisting}[language=python, caption={Code excerpt showing how OpenSlide and DeepZoomGenerator are used to obtain tiles of specified dimension and resolution level from an input WSI image}, label= lst:tiling-code]
    ...
    for row in range(rows):
            for col in range(cols):
                ...
                if temp_tile.std() > 15 and temp_tile.mean() < 230 and temp_tile.shape == (96,96,3):
                    tiff.imsave(save_path, temp_tile)
\end{lstlisting}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.2\textwidth}
        ...
        \label{fig:e-tile}
    \end{subfigure}
    \caption{Illustrating effect of Macenko normalization on the base H\&E stained tile. Additionally shows the H-component tile with prominently highlighted nuclei and the E-component highlighting cytoplasmic distribution}\label{fig:macenko code example}
\end{figure}

 As shown below in listing \ref{lst:tiling-normalized}, we extend the tiling pipeline by applying normalization function on each tile's Numpy array and save the normalized version. There is code to catch any convergence errors thrown by Numpy's \texttt{linalg.eigh} function that is often quite unstable and can fail when values are too high. This is the case with external and tissue edge tiles that contain large areas of white background but pass our mean intensity filter due to having high std deviation caused by presence of debris. 

\begin{lstlisting}[language=python, caption={Code excerpt showing modification made to the tiling pipeline to normalize each tile before saving. It calls \texttt{macenko\_norm\_HnE()} on each tile's numpy array and saves the normalized image. We also catch any SVD convergence errors thrown by tiles that have large areas of white space. This is the case for external tiles containing debris outside the main body of tissue.}, label= lst:tiling-normalized]
    for row in range(rows):
            for col in range(cols):
                temp_tile = np.array(tiles.get_tile(level_num, (col, row)).convert('RGB'))
                if temp_tile.std() > 15 and temp_tile.mean() < 230 and temp_tile.shape == (96,96,3):
...
Our datasets constitute of large volumes of images stored locally on disk in their base formats. The HDF5 format is usually very memory efficient when dealing with large datsets allowing us to load relevant slices of data into memory. However, we are required to perform pre-processing tasks on individual images and then feed the resulting images through the neural network. It would be highly memory inefficient to fetch images from the HDF5 dataset and preserve all pre-processed copies in memory until further processing. Therefore, all images are read from the disk, fed through the pre-processing pipeline described previously and written back after they have been processed. Listing \ref{lst:dataset-class}, displays the solution we implemented for continuous and efficient data fetching from disk. We utilize a dedicated dataset component, to fetch processed data from disk and create a stream of images the CNN can readily process without loading any image to memory. The dataset allows 

\begin{lstlisting}[ language=python, caption={custom \texttt{breastCancerDataset} class to fetch all training and validation images and ground truth labels from a given base directory. It creates objects associating each image's path with its corresponding label to make a stream of data that can be read by pytorch's CNN dataloaders}, label= lst:dataset-class]

...
We have undertaken a parameter space exploration task to identify the most optimal configuration for our model, particularly the activation function, optimizer, batch size and dropout rate. We have utilized PyTorch's \texttt{RayTune} package to automatically perform a combinatorial parameter space exploration. It takes as input all possible parameter values we want to test and records the performance of a specified number of models, each trained with a random parameter combination. We avoided exhaustively testing every possible model setting as that was causing a "parameter space explosion", crashing the entire process due to overloaded GPU memory. Using the model's validation loss as the chosen performance metric, we have identified the most optimal configuration  for our final model that we illustrated in section \ref{cnn-architecture}. 
